...
package edu.nyu.cs.assignment6.tests;

// import the class to be tested
import edu.nyu.cs.assignment6.Moped;
...
public class MopedTest {
        ...
        public void init() {
                ...
                try {
                        ...
                        assertEquals("Expected to be able to instantiate a new Moped object using a no-args constructor",
                                        "In fact, we are unable to instantiate a Moped object using a no-args constructor.");
                }
        }
        ...
        public void implementMovement(int[] mockLocation, String mockMovementType) {
                // System.out.println(String.format("Trying moped move %s from original point %s
                // facing %s", mockMovementType, Arrays.toString(m.getLocation()),
                // m.getOrientation()));
                try {
                        ...
                        assertEquals(
                                        String.format("Expected program not to crash when trying to move %s.",
                                                        mockMovementType),
                                        String.format("Program crashed when moving %s to %s: %s", mockMovementType,
                                                        Arrays.toString(mockLocation), e));
                }
        }
        ...
        public void testInitialLocation() {
                ...
                if (!Arrays.equals(actualLocation, expectedLocation)) {
                        assertEquals(
                                        String.format("Expected the moped's initial location to be %s.",
                                                        Arrays.toString(expectedLocation)),
                                        String.format(
                                                        "In fact, the initial location of your moped is %s.  FYI, this is the value returned by the getLocation() method.",
                                                        Arrays.toString(actualLocation)));
                }
        }
                ...
                if (!Arrays.equals(actualLocation, expectedLocation)) {
                        assertEquals(
                                        String.format("Expected the moped's initial location to be %s.",
                                                        Arrays.toString(expectedLocation)),
                                        String.format(
                                                        "In fact, the initial location of your moped is %s.  Make sure you are not copying a reference to an array.",
                                                        Arrays.toString(actualLocation)));
                }
        }
        ...
        public void testInitialOrientation() {
                ...
                if (!expectedOrientation.equals(actualOrientation)) {
                        assertEquals(
                                        String.format("Expected the moped's initial orientation to be '%s'.",
                                                        expectedOrientation),
                                        String.format(
                                                        "In fact, the initial orientation of your moped is '%s'.  FYI, this is value is the value returned by the getOrientation() method.",
                                                        actualOrientation));
                }
        }
         ...
         * 
         * @param mockOrientation   The current moped orientation (i.e. "north",
         *                          "south", "east", "west")
         * @param movements         A String array indicating a series of movements to
         *                          perform (i.e. "straight", "backwards", "left",
         *                          "right")
         * @param mockLocations     A two-dimensional array of coordinates from which to
         *                          start.
         * @param expectedLocations A two-dimensional array of coordinates where we
         *                          expect the moped to end up.
         */
        public TestOutcome testMovement(String mockOrientation, String[] mockMovementSequence, int[][] mockLocations,
                        int[][] expectedLocations, String[] expectedOrientations) {
                ...
                for (int i = 0; i < mockLocations.length; i++) {
                        ...
                        // not all tests have a specific expectation of orientation.. .leave blank if
                        // none
                        String expectedOrientation = "any orientation"; // will hold expected orientation at end, if any
                                                                        // has been
                                                                        // specified
                        if (expectedOrientations.length > 0)
                                expectedOrientation = expectedOrientations[i];
                        // System.out.printf("start location: %s; orientation: %s; moves: %s;\n",
                        // Arrays.toString(mockLocation), mockOrientation,
                        // Arrays.toString(mockMovementSequence));
                        ...
                        try {
                                ...
                                String actualMsg = String.format(
                                                "Program crashed when instantiating or when setLocation or setOrientation were called location to put moped at %s facing %s: %s",
                                                Arrays.toString(mockLocation), mockOrientation, e);
                                o = new TestOutcome(false, expectedMsg, actualMsg);
                        }
                        // implement the movements
                        for (String mockMovementType : mockMovementSequence) {
                                implementMovement(mockLocation, mockMovementType); // do the movement
                        }
                        ...
                        try {
                                ...
                                boolean isSameOrientation = expectedOrientation.equals(actualOrientation)
                                                || expectedOrientation.equals("any orientation"); // check for expected
                                                                                                  // orientation, if
                                                                                                  // specified. If "any"
                                                                                                  // allowed, ignore.
                                // System.out.printf("end location: %s; orientation: %s; expectedLocation: %s;
                                // expectedOrientation: %s\n", Arrays.toString(actualLocation),
                                // actualOrientation, Arrays.toString(expectedLocation), expectedOrientation);
                                if (!isSameLocation || !isSameOrientation) {
                                        // failure
                                        String expectedMsg = String.format(
                                                        "Expected a moped facing '%s' going %s from %s to end up at %s facing '%s'.",
                                                        mockOrientation, Arrays.toString(mockMovementSequence),
                                                        Arrays.toString(mockLocation),
                                                        Arrays.toString(expectedLocation), expectedOrientation);
                                        String actualMsg = String.format(
                                                        "In fact, the moped ended up at %s facing '%s'.  FYI, these incorrect location and/or orientation values were returned by the moped's getLocation() and getOrientation() methods, respectively.",
                                                        Arrays.toString(actualLocation), actualOrientation);
                                        o = new TestOutcome(false, expectedMsg, actualMsg);
                                } else {
                                        // success
                                ...
                                String actualMsg = String
                                                .format("Program crashed when moving calling getLocation and/or getOrientation: ",
                                                                e);
                                o = new TestOutcome(false, expectedMsg, actualMsg);
                        }
                }
        ...
        public TestOutcome testGoStraightFacingNorthward() {
                ...
                TestOutcome o = testMovement(mockOrientation, mockMovements, mockLocations, expectedLocations,
        ...
        public TestOutcome testGoLeftFacingNorthward() {
                ...
                int[][] expectedLocations = mockLocations;
                String[] expectedOrientations = {
                                "west",
                                "west",
                                "west",
        ...
        public TestOutcome testMultiStepMovementsNorthwards() {
                String mockOrientation = "north";
                String[] mockMovements = { "right", "straight", "left", "straight", "left", "straight", "right",
        ...
        public void testGasUsage() {
                ...
                String[][] mockMovements = {
                                { "straight", "left", "straight" }, // {10, 5},
                                { "right", "straight", "left", "straight", "straight" }, // {125, 10},
                                { "left", "straight", "left", "straight", "left", "straight", "right", "straight",
                                                "straight" }, // {2, 1},
                                { "straight", "backwards", "straight", "straight", "left", "straight", "right",
                                                "straight" } // {200, 2}
                };
                ...
                for (int i = 0; i < mockMovements.length; i++) {
                        ...
                        try {
                                m = new Moped();
                                // System.out.println(String.format("setting location to %s",
                                // Arrays.toString(mockLocation)));
                                ...
                                for (String movement : mockMovement) {
                                        implementMovement(mockLocation, movement); // do the movement
                                }
                                ...
                                if (!isSame) {
                                        assertEquals(
                                                        String.format(
                                                                        "Expected gas levels to be %d%% after starting a moped at %s facing '%s', and then moving it %s.",
                                                                        expectedGasLevel, Arrays.toString(mockLocation),
                                                                        mockOrientation,
                                                                        Arrays.toString(mockMovement)),
                                                        String.format(
                                                                        "In fact, the moped ended up with gas level %d%%.  FYI, this value was returned by the getGasLevel() method.",
                                                                        actualGasLevel));
                                } else {
                                ...
                                assertEquals(
                                                ...
                                                String.format(
                                                                "Program crashed when instantiating an object, starting it facing %s at %s, moving %s and then checking gas levels: %s",
                                                                mockOrientations, Arrays.toString(mockLocation),
                                                                Arrays.toString(mockMovement), e));
                        }
                }
        }
        ...
        public void testIsParked() {
                ...
                for (int i = 0; i < mockMovements.length; i++) {
                        ...
                        String mockOrientation = mockOrientations[i];
                        String[] mockMovement = mockMovements[i];
                        int[] expectedLocation = mockLocation;
                        String expectedOrientation = mockOrientation;
                        try {
                                m = new Moped();
                                // System.out.println(String.format("setting location to %s",
                                // Arrays.toString(mockLocation)));
                                ...
                                if (!mockLocation.equals(expectedLocation) && !mockOrientation.equals(expectedOrientation)) {
                                        assertEquals(
                                                        String.format(
                                                                        "Parked car shouldn't have moved from %d pointing at %s" ,
                                                                        mockLocation, Arrays.toString(mockLocation),
                                                                        mockOrientation),"");
                                }
                                int actualGasLevel = m.getGasLevel();
                                if(actualGasLevel != 100) {
                                        assertEquals(String.format("Parked car should not consume gas, was at %d%%", actualGasLevel),"");
                                ...
                                assertEquals(
                                                "Expected program not to crash when instantiating a moped, moving it and checking gas levels.",
                                                String.format(
                                                                "Program crashed when instantiating an object and moving it around while parked",
                                                                mockOrientations, Arrays.toString(mockLocation),
                                                                Arrays.toString(mockMovement), e));
                        }
                }
